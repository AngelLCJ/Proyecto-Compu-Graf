...
void Camera::keyControl(bool* keys, GLfloat deltaTime)
{
    ...
    if (keys[GLFW_KEY_P] && !aerialCamera) {
        ...
        // Ajustar la posición de la cámara aérea más arriba en el eje Y
        position.y += 10.0f; // Ajusta según tu necesidad
        // Ajustar la orientación de la cámara para mirar hacia abajo
        pitch = -89.0f;
        // Ajustar la dirección frontal de la cámara
        front = glm::vec3(0.0f, -1.0f, 0.0f);
    }
    else if (keys[GLFW_KEY_P] && aerialCamera) {
        aerialCamera = false;
        position = lastCameraPosition;
        // Restaurar la orientación y la dirección frontal de la cámara
        pitch = 0.0f;
        front = glm::vec3(0.0f, 0.0f, -1.0f);
    }
}
